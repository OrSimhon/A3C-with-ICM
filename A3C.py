...
class ActorCritic(nn.Module):
    def __init__(self, input_dims, n_actions, gamma=0.99, lamda=0.98):
        super().__init__()
    ...
    def calc_loss(self, new_states, hx, done, rewards, values, log_probs, r_i_t=None):
        ...
        if r_i_t is not None:
            rewards += r_i_t.detach().numpy()  # add intrinsic reward
        returns = self.calc_R(done, rewards, values)

        # GAE Calculation
        next_v = T.zeros(1, 1) if done else self.forward(T.tensor(new_states, dtype=T.float).unsqueeze(0), hx)[1]
        ...
        for t in range(n_steps):  # for each step
            for k in range(0, n_steps - t):  # for that step to the end
                temp = (self.gamma * self.lamda) ** k * delta_t[t + k]
                gae[t] += temp
        ...
        entropy_loss = (-log_probs * T.exp(log_probs)).sum()  # pi_entropy = sum(-log(pi) * pi)

        # critic loss
        critic_loss = F.mse_loss(values[:-1].squeeze(), returns)

        total_loss = actor_loss + critic_loss - 0.01 * entropy_loss # from the paper
        return total_loss
